        ...
        for cond=1:length(welkexoconditions)
           ...
           trials = fieldnames(subjectgaitcycles.(genvarname(subject)).(genvarname(condition)));
            ...
            for trial=1:length(trials)
                ...
                tempfile = strcat(trialdir, '/analyzemuscles_100conmuscletrack_BodyKinematics_pos_global.sto');
...
%                 timespercent = (times - times(1)) ./ (times(end) - times(1)) .*100;
                ...
                % here we only really need COM Y values. - hard code
                coord = char('center_of_mass_Y');
                tempcol = tempTimeSeriesTable.getDependentColumn(java.lang.String(coord)).getAsMat();
                % get the times (rows) that we want 
                if row_idx_start == 0
                    col = tempcol(row_idx_start+1:row_idx_end+1);
                else
                    col = tempcol(row_idx_start:row_idx_end);
                end
                % interpolate to match percentage gait cycle
...
%                 timespercent = (times - times(1)) ./ (times(end) - times(1)) .*100;
                ...
                % interpolate to match gait percentage
                tempcolinterp = interp1(timespercent, col, timespercent101);
                % add to struct
...
%         end
    ...
    tempfig2 = figure('Position',[1,1,600,200]);
    exocolor = '#AB82FF';
    natcolor = '#FF7F00';
...
%             plot(welkexostruct.time, mean(muscleplot_exo,2), 'b:','LineWidth',0.4);%char(markb(subj)))
        ...
        xlabel('% gait cycle')
        ylabel('Center of Mass Height [m]')
%         grid on;
    end
%     subplot(5,7,i);
...
%     plot(welkexostruct.time, mean(muscleplot_exo,2), char(markb(subj)))
    legend('Natural','Exotendon','location','southeast')

    print(tempfig2, ...
        strcat('G:\Shared drives\Exotendon\muscleModel\analysis\', tempthing, 'COM_Y_combined', '.png'),...
        '-dpng', '-r500')
    disp('print 2')
end