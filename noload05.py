...
def add_to_study(study):
    ...
    static_trial = static.add_trial(1, omit_trial_dir=False)
    # pdb.set_trace()
    ...
    ## slack condition
    slow = subject.add_condition('slow')
    ...
    slow_trial1 = slow.add_trial(1,
            # gait_events=gait_events,
            omit_trial_dir=False,
            )
    slow_trial2 = slow.add_trial(2,
            # gait_events=gait_events,
            omit_trial_dir=False)
    slow_trial3 = slow.add_trial(3,
            #gait_events=gait_events,
            omit_trial_dir=False)

    # ## free condition
    # ## slack condition
    # free = subject.add_condition('free')
    # # mat_file_temp = mat_file['stp_000']['non_tme_r']['x']['raw']
...
"""
    ...
    helpers.generate_ik_id_tasks(slow_trial1)
    helpers.generate_ik_id_tasks(slow_trial2)
    helpers.generate_ik_id_tasks(slow_trial3)
