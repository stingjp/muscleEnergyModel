    ...
    if subj[0:4] == 'wals':
        ...
        for cond in welkconditions:
            ...
            for keys in trials:
                ...
                # if trial 01 call function
                if keys == 'trial01':
                    # do the stuff
                    trialdir1 = trialdir

                # else copy the GRF from trial01 to here
                else:
                    filetocopy = os.path.join(trialdir1,'\\expdata\\GRFs.mot')
                    targetcopy = os.path.join(trialdir, '\\expdata\\GRFs.mot')
                    try:
