...
%                 timespercent = (times - times(1)) ./ (times(end) - times(1)) .*100;
    ...
    %                     col = tempcol(row_idx_start:row_idx_end);
                        ...
                        welkexostruct.(genvarname(coordshort)) = [welkexostruct.(genvarname(coordshort)), newtempcolinterp_r, newtempcolinterp_l];
                        exostruct_combine.(genvarname(coordshort)) = [exostruct_combine.(genvarname(coordshort)), newtempcolinterp_r, newtempcolinterp_l];
                        totalstruct_combine.(genvarname(coordshort)) = [totalstruct_combine.(genvarname(coordshort)), newtempcolinterp_r, newtempcolinterp_l];

                        totalsubjsstruct_combine.(genvarname(subject)) = [totalsubjsstruct_combine.(genvarname(subject)), newtempcolinterp_r, newtempcolinterp_l];
                    end
...
%                 timespercent = (times - times(1)) ./ (times(end) - times(1)) .*100;
    ...
    %                     col = tempcol(row_idx_start:row_idx_end);
                        ...
                        welknaturalstruct.(genvarname(coordshort)) = [welknaturalstruct.(genvarname(coordshort)), newtempcolinterp_r, newtempcolinterp_l];
                        naturalstruct_combine.(genvarname(coordshort)) = [naturalstruct_combine.(genvarname(coordshort)), newtempcolinterp_r, newtempcolinterp_l];
                        totalstruct_combine.(genvarname(coordshort)) = [totalstruct_combine.(genvarname(coordshort)), newtempcolinterp_r, newtempcolinterp_l];
...
%                     end
    ...
    % welkallcoordsexo_combine_mean = struct();
    welkallcoordsexo_combine_std = struct();
    ...
    for subj=1:length(subjs)
        ...
        % welkallcoordsexo_combine_mean.(genvarname(subject)) = mean([ ...
        ...
        %     ]);
        for c=2:length(coordinates)
            
            co = char(coordinates(c));
            welkallcoordsexo_combine_std.(genvarname(subject)).(genvarname(co)) = ...
            mean(std(welkexostruct_combine.(genvarname(subject)).(genvarname(co)),0,2));

        % welkallcoordsexo_combine_std.(genvarname(subject)) = [ ...
        %     mean(std(welkexostruct_combine.(genvarname(subject)).hip_flexion,1)), ...
        %     mean(std(welkexostruct_combine.(genvarname(subject)).hip_adduction,1)), ...
        %     mean(std(welkexostruct_combine.(genvarname(subject)).hip_rotation,1)), ...
        %     mean(std(welkexostruct_combine.(genvarname(subject)).knee_angle,1)), ...
        %     mean(std(welkexostruct_combine.(genvarname(subject)).ankle_angle,1)), ...
        %     mean(std(welkexostruct_combine.(genvarname(subject)).subtalar_angle,1)), ...
        %     ];
        end
    end
    % same for natural
    % welkallcoordsnat_combine_mean = struct();
    welkallcoordsnat_combine_std = struct();
    ...
    for subj=1:length(subjs)
        ...
        for c=2:length(coordinates)
            co = char(coordinates(c));
            welkallcoordsnat_combine_std.(genvarname(subject)).(genvarname(co)) = ...
            mean(std(welknaturalstruct_combine.(genvarname(subject)).(genvarname(co)),0,2));
        end
    end


    % okay so I have the std for each subject for each coordinate, for each condition. Think about combining these better. 
    keyboard
    ...
    % % figure out how to get the std for these as well -> this actually doesn't work - have to do it by coordinate - they have different ranges etc. 
    ...
    %     % each subject
    %     totalmeansacrosscoords.(genvarname(subject)) = mean(totalsubjsstruct_combine.(genvarname(subject))(:));
    %     totalstdsacrosscoords.(genvarname(subject)) = std(totalsubjsstruct_combine.(genvarname(subject))(:));
    % end
    ...
    % % find the coordinate with max MAE across all subjects and conditions -> I think this is wrong too. I think you have to do by subject, by coordinate
